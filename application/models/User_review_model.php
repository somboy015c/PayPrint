...
class User_review_model extends CI_Model
{
    ...
    public function add_review()
    {
        ...
        if (!empty($data['seller_id']) && !empty($data['user_id']) && !empty($data['rating'])) {
            ...
            $reviews = $this->get_reviews($data['seller_id']);
            if (!empty($reviews)) {
                $count = count($reviews);
                $total = 0;
                ...
                $da['rating'] = round($total / $count);
                $da['rated'] = $total;
            } else {
                $da['rating'] =  0;
                $da['rated'] =  0;
            }
            $this->db->where('id', $data['seller_id']);
    ...
    public function get_top_rating()
    {
        $this->db->join('users', 'users.id = user_reviews.user_id');
        $this->db->select('user_reviews.*, users.username as user_username, users.slug as user_slug');
        $this->db->order_by('user_reviews.rating', 'DESC');
        $query = $this->db->get('user_reviews');
        return $query->result();
    }
    ...
    public function delete_review($id)
    {
        ...
        if (is_admin() || $data["review"]->user_id == user()->id) {
            $this->db->where('id', $id);
            if ($this->db->delete('user_reviews')){
                $reviews = $this->get_reviews($data["review"]->seller_id);
                if (!empty($reviews)) {
                    $count = count($reviews);
                    $total = 0;
                    foreach ($reviews as $review) {
                ...
                $this->db->where('id', $data["review"]->seller_id);
