...
class Order_model extends CI_Model
{
	...
	//get unread sales
	public function get_unread_sales_count($user_id)
	{
		$user_id = clean_number($user_id);
		$this->db->join('order_products', 'order_products.order_id = orders.id');
		$this->db->select('orders.id');
		...
		$this->db->where('order_products.seller_id', $user_id);
		$this->db->where('order_products.order_status !=', 'completed');
		$this->db->where('order_products.is_view', 0);
		$query = $this->db->get('orders');
		return $query->num_rows();
	}

	//set sales viewed
	public function set_sales_viewed($user_id, $order_id)
	{
		$user_id = clean_number($user_id);
		$order_id = clean_number($order_id);
		$data = array(
	...
	//get ordered sales count
	public function get_ordered_sales_count($user_id)
	{
		$user_id = clean_number($user_id);
		$this->db->join('order_products', 'order_products.order_id = orders.id');
		$this->db->select('orders.id');
	...
	public function decrease_product_quantity_after_sale($order)
	{
		...
		if (!empty($order_products)) {
			foreach ($order_products as $order_product) {
				...
				//decrease balance after sale
				if ($order->payment_method == 'wallet' || $order->payment_method == 'earnings') {
					$user = get_user($order->buyer_id);
					if (!empty($user)) {
						if ($order->payment_method == 'wallet') {
							$dati['wallet'] = $user->wallet - $order->price_total;
						}elseif ($order->payment_method == 'earnings') {
							$dati['balance'] = $user->balance - $order->price_total;
						}
						$this->db->where('id', $user->id);
						$this->db->update('users', $dati);
					}
				}
			}
		}
