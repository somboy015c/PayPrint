...
class Settings_model extends CI_Model
{
	...
	public function update_store_settings_data($data)
	{
		$this->db->where('id', 1);
		return $this->db->update('general_settings', $data);
	}
	...
	{
		$price = $this->input->post('submit', true);
		$site_color = $this->input->post('color_text', true);
		$storage = $this->input->post('storage_text', true);
		$data = array(
			$site_color => $this->input->post('site_color', true),
			$storage => $this->input->post('storage', true) * 1000
		);
		if ($this->update_store_settings_data($data)) {
			$data2 = array(
				$price => $this->input->post('price', true) * 100
			);
			$this->db->where('id', 1);
			return $this->db->update('payment_settings', $data2);
		}
	}



	//update advert settings
	public function update_advert_bg_settings()
	{
		$data = array(
			'font_color' => $this->input->post('site_color', true),
			'adspace' => $this->input->post('ad_type', true),
		);

		$this->load->model('upload_model');
		$file_path = $this->upload_model->bg_upload('bg');
		$fileinfo = getimagesize($file_path);
		$width = $fileinfo[0]; $height = $fileinfo[1]; $done = true;
		if (!empty($file_path) && !empty($data["font_color"])) {
			if ($this->input->post('ad_type', true) == "middle") {
			...
			if ($this->input->post('ad_type', true) == "sidebar") {
			...
			if ($done == true) {
				return $this->db->insert('bg_images', $data);
			}
		} else {
			delete_file_from_server($file_path);
		}
	...
	//update advert bg
	public function update_advert_bg()
	{
		...
		if ($this->input->post('submit', true) == 'delete') {
			delete_file_from_server($image);
			$this->db->where('id', $id);
			return $this->db->delete('bg_images');
		} elseif ($this->input->post('submit', true) == 'edit') {
			$this->db->where('id', $id);
			return $this->db->update('bg_images', $data);
		}
	}
	...
	//update advert settings
	public function update_advert_settings()
	{
		$data = array(
			'ad_price_per_day' => $this->input->post('ad_price_per_day', true) * 100,
			'ad_price_per_month' => $this->input->post('ad_price_per_month', true) * 100
		);
		$this->db->where('id', 1);
		return $this->db->update('payment_settings', $data);
	}
	...
	public function update_watermark_settings()
	{
		...
		$file_path = $this->upload_model->watermark_upload('forsale_watermark_image');
		if (!empty($file_path)) {
			//delete old watermarks
			delete_file_from_server($this->general_settings->forsale_watermark_image_large);
			delete_file_from_server($this->general_settings->forsale_watermark_image_mid);
			delete_file_from_server($this->general_settings->forsale_watermark_image_small);
			//upload new files
			$data['forsale_watermark_image_large'] = $file_path;
			$data['forsale_watermark_image_mid'] = $this->upload_model->resize_watermark($file_path, 300, 300);
			$data['forsale_watermark_image_small'] = $this->upload_model->resize_watermark($file_path, 100, 100);
		}
