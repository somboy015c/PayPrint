...
class Earnings_model extends CI_Model
{
	...
	public function add_seller_earnings($order_product)
	{
		if (!empty($order_product)) {
			...
			if (!empty($order)) {
				...
				//delete order product if seller is member
				if (!empty($user)) {
					if ($user->role == "member") {
						$data = array(
							'is_deleted' => 1
	...
	public function calculate_earned_amount($order_product)
	{
		if (!empty($order_product)) {
			$user = get_user($order_product->seller_id);
			$price = $order_product->product_unit_price * $order_product->product_quantity;
			if ($user->role == "member") { $earned = $price - (($price * $order_product->commission_rate) / 100); }else{$earned = $price;}
	...
	public function get_user_payout_account($user_id)
	{
		...
		if (!empty($row)) {
			...
			$data = array(
				...
				'ban_bank_account_holder_name' => "",
				'ban_bank_branch_country_id' => "",
				'ban_bank_name' => "",
				'is_payout' => "",
				'ban_number' => ""
			);
	...
	//set ban payout account
	public function set_ban_payout_account($user_id)
	{
		$user_id = clean_number($user_id);
		$data = array(
			'ban_bank_account_holder_name' => $this->input->post('ban_bank_account_holder_name', true),
			'ban_bank_branch_country_id' => $this->input->post('ban_bank_branch_country_id', true),
			'ban_bank_name' => $this->input->post('ban_bank_name', true),
			'ban_number' => $this->input->post('ban_number', true),
			'is_payout' => 1
		);
		$this->db->where('user_id', $user_id);
		return $this->db->update('user_payout_accounts', $data);
