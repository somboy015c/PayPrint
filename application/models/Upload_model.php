...
class Upload_model extends CI_Model
{
	...
	public function avatar_upload($path)
	{
		...
		try {
			$image = new ImageResize($path);
			$image->quality_jpg = 84;
			/*$image->crop(240, 240, true);*/
			$new_path = 'uploads/profile/user_avatar_' . generate_unique_id() . '.png';
	...
	//banner image upload
	public function banner_upload($path)
	{

		try {
			$image = new ImageResize($path);
			$image->quality_jpg = 85;
			$image->crop(1200, 410, true);
			$new_path = 'uploads/banner/banner_' . generate_unique_id() . '.jpg';
			$image->save(FCPATH . $new_path, IMAGETYPE_JPEG);
			return $new_path;
		} catch (ImageResizeException $e) {
			return null;
	...
	//ad image upload
	public function user_ad_upload($path)
	{
		try {
			$image = new ImageResize($path);
			$image->quality_jpg = 84;
			$new_path = 'uploads/ad/user_ad_' . generate_unique_id() . '.jpg';
			$image->save(FCPATH . $new_path, IMAGETYPE_PNG);
			return $new_path;
		} catch (ImageResizeException $e) {
			return null;
	...
	//work image upload
	public function work_upload($path)
	{
		try {
			$image = new ImageResize($path);
			$image->quality_jpg = 84;
			$new_path = 'uploads/cv/user_work_' . generate_unique_id() . '.png';
			$image->save(FCPATH . $new_path, IMAGETYPE_PNG);
			return $new_path;
		} catch (ImageResizeException $e) {
			return null;
	...
	//ad bg image upload
	public function bg_upload($file_name)
	{
		if ($this->input->post('ad_type', true) == 'middle') {
			$config['upload_path'] = './uploads/ad/bg_images/middle/';
			$config['allowed_types'] = 'gif|jpg|jpeg|png|svg';
			...
			if ($this->upload->do_upload($file_name)) {
				$data = array('upload_data' => $this->upload->data());
				if (isset($data['upload_data']['full_path'])) {
					return 'uploads/ad/bg_images/middle/' . $data['upload_data']['file_name'];
				}
			}
			return null;
		} else {
			$config['upload_path'] = './uploads/ad/bg_images/sidebar/';
			$config['allowed_types'] = 'gif|jpg|jpeg|png|svg';
			$config['file_name'] = 'bg_' . uniqid();
			$this->load->library('upload', $config);
			...
			if ($this->upload->do_upload($file_name)) {
				$data = array('upload_data' => $this->upload->data());
				if (isset($data['upload_data']['full_path'])) {
					return 'uploads/ad/bg_images/sidebar/' . $data['upload_data']['file_name'];
				}
			}
			return null;
		}
	...
	//user watermark upload
	public function user_watermark_upload($file_name)
	{
		...
		if ($this->upload->do_upload($file_name)) {
			$data = array('upload_data' => $this->upload->data());
			if (isset($data['upload_data']['full_path'])) {
				return 'uploads/watermark/' . $data['upload_data']['file_name'];
			}
		}
		return null;
	}
	...
	//resize watermark
	public function user_resize_watermark($path, $width, $height)
	{
		try {
			$image = new ImageResize($path);
			$image->resizeToWidth($width);
			$new_name = 'watermark_' . generate_unique_id() . '.png';
			$new_path = 'uploads/watermark/' . $new_name;
			$image->save(FCPATH . $new_path, IMAGETYPE_PNG);
			return 'uploads/watermark/' . $new_name;
		} catch (ImageResizeException $e) {
			return null;
		}
	}
	...
	public function add_watermark($image_path, $watermark_size)
	{
		...
		if ($post_type != 'for_sale') {
			if (user()->role == 'admin') {
				...
				$watermark = user()->watermark_image_large;
				if ($watermark_size == 'mid') {
					$watermark = user()->watermark_image_mid;
				}
				if ($watermark_size == 'small') {
					$watermark = user()->watermark_image_small;
				}
			}
		} else {
			$watermark = $this->general_settings->forsale_watermark_image_large;
			if ($watermark_size == 'mid') {
				$watermark = $this->general_settings->forsale_watermark_image_mid;
			}
			if ($watermark_size == 'small') {
				$watermark = $this->general_settings->forsale_watermark_image_small;
			}
		}
