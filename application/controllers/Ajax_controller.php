...
class Ajax_controller extends Home_Core_Controller
{
	...
	*/
	...
	public function search_location()
	{
		...
		$country = $this->session->userdata('modesy_default_location');
		$state = $this->session->userdata('modesy_default_state');
		$data = array(
			'result' => 0,
			'response' => ''
		);
		...
		if (!empty($input_value)) {
			...
			if (!empty($countries) && empty($country)) {
				$data['result'] = 1;
			...
			$data['result'] = 1;
				...
				if (!empty($members)) { $count =0;
					foreach ($members as $member) {
						if ($member->role == 'vendor' || ($member->role == 'member' && $member->is_workshop == 1)) { $count++;
							$response .= '<li><a href="' . $lang_base_url . 'profile' . '/' . $member->slug . '">' . $member->shop_name . '</a></li>';
						}
					}
					if ($count == 0) {
