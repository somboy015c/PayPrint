...
class Earnings_admin_controller extends Admin_Core_Controller
{
    ...
    public function seller_balances()
    {
        ...
        if ($data["req"] == 'seller') {
            $data['title'] = ("Credit Requests");
            $data['form_action'] = admin_url() . "seller-balances?req=seller";
            //get paginated credit request
            $pagination = $this->paginate(admin_url() . 'seller-balances?req=seller', $this->earnings_admin_model->get_wallet_requests_count());
            $data['requests'] = $this->earnings_admin_model->get_paginated_requests($pagination['per_page'], $pagination['offset']);
        }else{
        ...
        if (!empty($data["req"]) && $data["req"] != 'seller') {
            redirect($this->agent->referrer());
        }

        $this->load->view('admin/includes/_header', $data);
        if ($data["req"] == 'seller') {
            $this->load->view('admin/earnings/credit_request', $data);
        }else{
    ...
    public function approve_credit()
    {
        $user_id = $this->input->post('user_id', true);
        $amount = $this->input->post('amount', true);

        if ( $this->earnings_admin_model->approve_credit($user_id, $amount)) {
    ...
    public function add_payout_post()
    {
        ...
        //check user balance
        if (!$this->earnings_admin_model->check_user_balance($user_id, $amount, $payout_from)) {
