...
<?php $query_string = create_product_filters_query_string();  if (!empty($search_type)){ $drls = '&drls=' . $search_type; }else { $drls =''; }?>
...
<div class="col-12 col-md-3 sidebar-products">
	<div id="collapseFilters" class="product-filters">
		...
		<?php endif; ?>
		<?php if (!empty($categories)):  if ((empty($category) && $search_type != 'Handwork') || (!empty($category) && ($category->name != 'Services' && $category->parent_id != 12))): ?>
			<div class="filter-item">
				<div class="filter-list-container">
					<ul class="filter-list filter-custom-scrollbar">
						<?php foreach ($categories as $item): if ($item->id != 12): ?>
							<li>
								<div class="left">
									<div class="custom-control custom-checkbox">
										...
										<label for="cat_<?php echo $item->id; ?>" class="custom-control-label" onclick="window.location.href= '<?php echo generate_category_url($item) . $query_string . $drls; ?>'"></label>
									</div>
								</div>
								<div class="rigt">
									<label for="cat_<?php echo $item->id; ?>" class="checkbox-list-item-label" onclick="window.location.href= '<?php echo generate_category_url($item) . $query_string . $drls; ?>'"><?php echo html_escape(get_category_name_by_lang($item->id, $this->selected_lang->id)); ?></label>
								</div>
							</li>
					...
					<ul class="filter-list filter-custom-scrollbar">
						<?php foreach ($subcategories as $item): ?>
							<li>
								<div class="left">
									<div class="custom-control custom-checkbox">
										<input type="radio" class="custom-control-input" id="cat_<?php echo $item->id; ?>" value="<?php echo $item->id; ?>" <?php echo (!empty($subcategory) && $subcategory->id == $item->id) ? 'checked' : ''; ?>>
										<label for="cat_<?php echo $item->id; ?>" class="custom-control-label" onclick="window.location.href= '<?php echo generate_category_url($item) . $query_string . $drls; ?>'"></label>
									</div>
								</div>
								<div class="rigt">
		...
		if ($show_location_filter == true):
			...
			<?php if ($this->form_settings->product_conditions == 1 && !empty($category) && ((empty($category) && $search_type != 'Handwork') || (!empty($category) && ($category->name != 'Services' && $category->parent_id != 12)))): ?>
			<div class="filter-item">
		...
		<?php if ($this->form_settings->price == 1 && ((empty($category) && $search_type != 'Handwork') || (!empty($category) && ($category->name != 'Services' && $category->parent_id != 12)))):
