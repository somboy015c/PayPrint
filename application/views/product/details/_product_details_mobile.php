...
<?php if ($user->role != 'member' || ($user->role == 'member' && empty($product->post_type))): ?>
	...
	<div class="row-custom price" style="margin-bottom: 15px;">
		<span style="font-weight: 100; font-size: 14px;"><?php if ($product->qty_scale == ' (Pieces)') { $cond = ', (Pieces)'; echo "At Retail Price"; } elseif ($product->qty_scale == ' (Bulk)') { $cond = ', ' . $product->qty_scale . ', ' . $product->bulk_quantity . ' Pieces In A Bulk'; echo "At Wholesale Price"; } elseif ($product->qty_scale == ' (Company Sales)') {$cond = ''; echo "At Company's Price";} else {$cond = '';} ?></span>
	</div>
			...
			<div class="item-details">
				...
				<div class="right">
					...
					if (!empty($product_condition)):?>
						<span><?php echo html_escape($product_condition->option_label) . $cond; ?></span>
					<?php endif; ?>
				</div>
			</div>
		<?php endif; ?>
...
<?php else: ?>
	...
	<div class="row-custom price">
		<div class="row-custom meta">
				<b style="font-size: 16px;"><?php echo 'Hand Made Work'; ?></b>
				<?php if ($general_settings->product_reviews == 1): ?>
					<span><i class="icon-comment"></i><?php echo html_escape($comment_count); ?></span>
				<span><i class="icon-heart"></i><?php echo get_product_favorited_count($product->id); ?></span>
				<?php endif; ?>
		...
		<div class="row-custom meta">
				<b style="font-size: 16px;"><?php echo trans("by"); ?></b>&nbsp;<a href="<?php echo lang_base_url() . 'profile' . '/' . $product->user_slug; ?>"><?php echo character_limiter(get_shop_name_product($product), 30, '..'); ?></a>
			<?php if (auth_check()): ?>
				<button class="btn btn-contact-seller" data-toggle="modal" data-target="#messageModal"><i class="icon-envelope"></i> <?php echo trans("ask_question") ?></button>
			<?php else: ?>
				<button class="btn btn-contact-seller" data-toggle="modal" data-target="#loginModal"><i class="icon-envelope"></i> <?php echo trans("ask_question") ?></button>
