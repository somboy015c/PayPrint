			...
			<?php foreach ($product_images as $image): ?>
				<div class="product-image-preview">
					<a class="image-popup lightbox" href="<?php echo get_product_image_url($image, 'image_big'); ?>" data-effect="mfp-zoom-out" title="">
						<img src="<?php echo get_product_image_url($image, 'image_default'); ?>" alt="<?php echo html_escape($product->title); ?>" class="img-fluid" <?php if (!empty($product->post_type)) {echo 'style="border-radius: 24px;"';} ?> >
					</a>
				</div>
				<?php break;
			endforeach; ?>
				...
				<div class="dots-container slider-dots <?php echo (count($product_images) < 2) ? 'hide-dosts-mobile' : ''; ?>">
					...
					<?php if (!empty($video)): ?>
						<button class="dot dot_slider">
							<img src="<?php echo get_product_image($product->id, 'image_small'); ?>" alt="dot" <?php if (!empty($product->post_type)) {echo 'style="border-radius: 24px;"';} ?> >
					...
					foreach ($product_images as $image): ?>
						<div class="item">
							<a class="image-popup lightbox" href="<?php echo get_product_image_url($image, 'image_big'); ?>" data-effect="mfp-zoom-out" title="">
								...
								<img src="<?php echo get_product_image_url($image, 'image_default'); ?>" class="img-product-slider" alt="<?php echo html_escape($product->title); ?>" <?php if (!empty($product->post_type)) {echo 'style="border-radius: 24px;"';} ?> >
							</a>
						</div>
					<?php endforeach;
				endif; ?>
