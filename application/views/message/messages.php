			...
			<?php else: ?>
						...
						<div class="row-custom messages-sidebar">
							<?php foreach ($unread_conversations as $item):
								...
								if ($item->receiver_id != $this->auth_user->id) {
									$user_id = $item->receiver_id; $string = 'Seller' . $item->receiver_id;
								} else {
									$user_id = $item->sender_id; $string = 'Buyer' . $item->sender_id;
								}
								...
								if (!empty($user)):?>
									<div class="conversation-item <?php echo ($item->id == $conversation->id) ? 'active-conversation-item' : ''; ?>">
										<a href="<?php echo lang_base_url(); ?>messages/conversation/<?php echo $item->id; ?>" class="conversation-item-link">
											...
											<?php else: ?>
												<div class="middle">
													<img src="<?php echo base_url() . "assets/img/user.png"; ?>" alt="<?php echo html_escape($string); ?>">
												</div>
												<div class="right">
													<div class="row-custom">
														<strong class="username"><?php echo html_escape($string); ?></strong>
														<label class="badge badge-success badge-new"><?php echo trans("new_message"); ?></label>
													</div>
													<div class="row-custom m-b-0">
														<p class="subject"><?php echo html_escape(character_limiter($item->subject, 28, '...')); ?></p>
				...
				<div class="col-sm-12 col-md-12 col-lg-9 col-message-content">
					...
					if ($this->auth_user->id == $conversation->sender_id) {
						$profile_id = $conversation->receiver_id; $string = 'Seller' . $conversation->receiver_id;
					} else {
						$string = 'Buyer' . $conversation->sender_id;
					}
					$profile = get_user($profile_id);
					if (!empty($profile)):?>
						<?php if ($conversation->type == 'message'): ?>
						...
						<?php else: ?>
							<div class="row-custom messages-head">
								<div class="sender-head">
									<div class="left">
										<img src="<?php echo base_url() . "assets/img/user.png"; ?>" alt="<?php echo html_escape($string); ?>" class="img-profile">
									</div>
									<div class="right">
										<strong class="username"><?php echo html_escape($string); ?></strong>
										<p class="p-last-seen">
					...
					<div class="row-custom messages-content">
							...
							<?php foreach ($messages as $item):
								if ($item->deleted_user_id != $this->auth_user->id): ?>
									<?php if ($this->auth_user->id == $item->receiver_id): 
										$m_c = $this->message_model->get_conversation($item->conversation_id)->type;?>
										<div class="message-list-item">
											...
											<?php else: ?>
												<div class="user-avatar">
													<div class="message-user">
														<img src="<?php echo base_url() . "assets/img/user.png"; ?>" alt="" class="img-profile">
													</div>
												</div>
												<div class="user-message">
													<div class="message-text">
						...
						<div class="message-reply">
							<!-- form start -->
							<?php if ($conversation->type == 'forsale') { $fs = 1; } else { $fs = 0; } ?>
							...
							<input type="hidden" name="for_sale" value="<?php echo $fs; ?>">
