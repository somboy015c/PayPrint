								...
								<?php if (!empty($quote_requests)): ?>
									<?php foreach ($quote_requests as $quote_request):
										...
										<tr>
											...
											<td><?php if ($quote_request->status == 'pending_payment'){ echo "Quote Accepted <br/> ("; } ?><?php echo trans($quote_request->status); ?><?php if ($quote_request->status == 'pending_payment'){ echo ")"; } ?></td>
											<td>
											...
											<td>
												<?php echo time_ago($quote_request->updated_at); ?>
												<?php if ($quote_request->viewer_id == $user->id && $quote_request->is_view == 0): ?>
													<span class="span-message-count" style="width: 30px;"><?php echo 'new'; ?></span>
												<?php $this->bidding_model->set_bidding_quotes_as_viewed($user->id, $quote_request->id); endif; ?>
												<?php if ($quote_request->viewer_id != $user->id && $quote_request->is_view == 1){ echo "<br/> (Seen)"; } ?>
											</td>
											<td>
					...
					<div class="modal-body">
						...
						<div class="form-group">
							<p class="calculated-price">
								...
								<strong><?php if ($quote_product->for_sale == 0) { $general_settings->commission_rate = 0; } echo trans("you_will_earn"); ?> (<?php echo get_currency($payment_settings->default_product_currency); ?>):&nbsp;&nbsp;
