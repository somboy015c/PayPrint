				...
				if (!empty($parent_categories)):
					foreach ($parent_categories as $category):
						if ($count <= $limit):?>
							<li class="nav-item dropdown" data-category-id="<?php echo $category->id; ?>">
								...
								<a href="<?php echo generate_category_url($category) . '?drls=cat'; ?>" class="bn-lg nav-link dropdown-toggle" style="min-width: 0px; min-height: 0px; text-align: left; width: unset;">
									<?php echo html_escape(get_category_name_by_lang($category->id, $this->selected_lang->id)); ?>
								</a>
								<a href="<?php echo generate_category_url($category) . '?drls=cat'; ?>" class="bn-md nav-link dropdown-toggle" style="font-size: 12px; min-width: 0px; min-height: 0px; text-align: left; width: unset;margin-right: 0px;">
									<?php echo html_escape(get_category_name_by_lang($category->id, $this->selected_lang->id)); ?>
								</a>
										...
										<div class="row">
												...
												<div class="card-columns">
													<?php foreach ($subcategories as $subcategory): ?>
														<div class="card">
															<div class="row">
																<div class="col-12">
																	<a href="<?php echo generate_category_url($subcategory) . '?drls=cat'; ?>" class="second-category"><?php echo html_escape($subcategory->name); ?></a>
																	<?php
											...
											<div class="col-4 col-category-images">
												<?php if (!empty($category->image_1) && $category->show_image_on_navigation == 1): ?>
													<div class="nav-category-image" style="width: 40%;">
														<a href="<?php echo generate_category_url($category) . '?drls=cat'; ?>">
												...
												<?php if (!empty($subcategories)):
													foreach ($subcategories as $subcategory):
														if (!empty($subcategory->image_1) && $subcategory->show_image_on_navigation == 1):?>
															<div class="nav-category-image" style="width: 40%;">
																<a href="<?php echo generate_category_url($subcategory) . '?drls=cat'; ?>">
														...
														if (!empty($third_categories)):
															foreach ($third_categories as $third_category):
																if (!empty($third_category->image_1) && $third_category->show_image_on_navigation == 1):?>
																	<div class="nav-category-image" style="width: 40%;">
																		<a href="<?php echo generate_category_url($third_category) . '?drls=cat'; ?>">
					...
					if (count($parent_categories) > $limit): ?>
						<li class="nav-item dropdown menu-li-more">
							<a class="bn-lg nav-link dropdown-toggle" data-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false" style="min-width: 0px; min-height: 0px; text-align: left; width: unset;"><?php echo trans("more"); ?></a>
							<a class="bn-md nav-link dropdown-toggle" data-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false" style="font-size: 12px; min-width: 0px; min-height: 0px; text-align: left; width: unset;margin-right: 0px;"><?php echo trans("more"); ?></a>
...
<?php else: ?>
	<div class="container" style="margin: 0px; padding: 0px;">
		<div class="navbar navbar-light navbar-expand" style="padding-left: 10px;">
			<ul class="nav navbar-nav mega-menu">
				<?php
				$limit = $general_settings->menu_limit;
				$menu_item_count = 1;
